...
export default function DynamicForm({
   ...
   const { fields: formFields, append, remove } = useFieldArray<FormFieldModel>({ control, name: 'fields' });
   ...
   const handleAddField = () => append(new FormFieldModel(formFields.length + 1));
   ...
   const handleFormSubmit = async (formData: FormDataModel) => {
      try {
         ...
         formData.fields = formData.fields.map((field) => ({
            ...field,
            required_field: Array.isArray(field.required_field) ? field.required_field.length > 0 : field.required_field,
         }));

         const response = await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/${id}`, formData);
   ...
   return (
      <form onSubmit={handleSubmit(handleFormSubmit)} className="flex flex-col items-center w-3/4 gap-10 p-5 border border-zinc-500">
               ...
               <div className="flex justify-between w-full">
                  <label htmlFor={`fields[${index}].question_type`} className="flex flex-col items-start w-3/4 gap-3">
                     ...
                     <select
                        ...
                        {...register(`fields[${index}].question_type` as const, { required: 'Please select a question type' })}
                     ...
                     >
                        <option value="" disabled>
                           {errors?.fields?.[index]?.question_type?.message || "Select question type"}
                        </option>
         ...
         <button type="button" onClick={handleAddField} className="flex items-center gap-1 text-lg font-bold text-green-500 transition-colors ease-in-out group hover:underline w-fit">
