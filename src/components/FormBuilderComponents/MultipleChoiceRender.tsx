   ...
   return (
      <div className="flex flex-col gap-3 py-5">
         {choices.map((choice, choiceIndex) => (
            <div key={choiceIndex} className='flex items-center justify-between gap-3'>
               <label
         ...
         <button type="button" onClick={onAddChoice} className="flex items-center gap-1 text-green-500 transition-colors ease-in-out group hover:text-green-600 hover:underline w-fit">
            <AddCircleOutlineOutlined className='transition-all ease-in-out origin-center group-hover:rotate-90' /> Add Choice
         </button>
      </div>
   );
}
